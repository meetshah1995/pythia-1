...
class HWC_feat_reader:
    def read(self, image_feat_path):
        ...
        #image_feat = np.transpose(tmp, axes=(2, 0, 1))
        image_feat = np.reshape(tmp, (-1, c_dim))
        return image_feat
...
class vqa_dataset(Dataset):
    def __init__(self, imdb_file, image_feat_directories, verbose=False, **data_params):
        ...
        self.image_max_loc = data_params['image_max_loc'] if 'image_max_loc' in data_params else None
        ...
        if data_version != imdb_version:
            print("observed imdb_version is", data_version, "expected imdb version is", imdb_version)
        ...
        if data_params['fastRead']:
            ...
            for feat_file in os.listdir(image_dir0):
                if feat_file.endswith("npy"):
                    image_feats = read_in_image_feats(self.image_feat_directories, self.image_feat_readers,
                                                      feat_file)
    ...
    def _get_image_features_(self, image_file_name):
        if self.fastRead:
            ...
            image_feats = read_in_image_feats(self.image_feat_directories,self.image_feat_readers, image_file_name)
