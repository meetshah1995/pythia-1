...
def prepare_data_set(imdb_file_label, image_dir_label,**data_config):
    ...
    if 'vocab_layout_file' in data_config:
        vocab_layout_file = os.path.join(data_root_dir, data_config['vocab_layout_file'])
        layout_max_len = data_config['layout_max_len'] if 'layout_max_len' in data_config else 13


    prune_filter_module = data_config['prune_filter_module'] if 'prune_filter_module' in data_config else False
    ...
    assert len(imdb_files) == len(image_feat_dirs), image_dir_label + "has different length with " + image_dir_label
    image_max_loc = data_config['image_max_loc'] if 'image_max_loc' in data_config else None
    ...
    for imdb_file_trn_name, image_feat_dir in zip(imdb_files,image_feat_dirs):
        ...
        image_feat_dirs = [os.path.join(data_root_dir, d) for d in image_feat_dir.split(',')]

        train_dataset = vqa_dataset(imdb_file=imdb_file_trn, image_feat_directories=image_feat_dirs,
