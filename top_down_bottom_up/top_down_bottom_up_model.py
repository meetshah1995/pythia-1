...
class top_down_bottom_up_model(nn.Module):
    ...
    def forward(self, image_feat_variable, input_question_variable, input_answers=None, image_text_feat_variable=None, **kwargs):
        ...
        if isinstance(image_feat_variable, list):
            ...
            attention = self.image_attention_model(image_feat_variable, question_embedding)
...
class vqa_multi_modal_model(nn.Module):
    def __init__(self, image_embedding_models_list, question_embedding_models, multi_modal_combine, classifier, image_feature_encode_list, inter_model=None, image_text_feat_embedding_models_list=None, image_text_feature_encode_list=None):
        super(vqa_multi_modal_model, self).__init__()
        self.image_embedding_models_list = image_embedding_models_list
        self.question_embedding_models = question_embedding_models
        self.image_text_feat_embedding_models_list = image_text_feat_embedding_models_list
    ...
    def forward(self, image_feat_variables, input_question_variable, image_dim_variable,input_answers=None, image_text_feat_variables=None, **kwargs):
        ...
        for i, image_feat_variable in enumerate(image_feat_variables):
            ...
            image_feat_variable_ft = self.image_feature_encode_list[i](image_feat_variable)

            image_embedding_models_i = self.image_embedding_models_list[i]
            for i_model in image_embedding_models_i:
                i_embedding = i_model(image_feat_variable_ft, question_embedding_total, image_dim_variable_use)
        ...
        if self.image_text_feature_encode_list is not None and self.image_text_feat_embedding_models_list is not None:
            ...
            for i, image_text_feat_variable in enumerate(image_text_feat_variables):
                image_dim_variable_use = None if i > 0 else image_dim_variable
                image_text_feat_variable_ft = self.image_text_feature_encode_list[i](image_text_feat_variable)

                image_text_feat_embedding_models_i = self.image_text_feat_embedding_models_list[i]
                for i_model in image_text_feat_embedding_models_i:
                    i_embedding = i_model(image_text_feat_variable_ft, question_embedding_total, image_dim_variable_use)
                    image_text_feat_embeddings.append(i_embedding)

            image_text_feat_embedding_total = torch.cat(image_text_feat_embeddings, dim=1)

        if self.inter_model is not None:
            image_embedding_total = self.inter_model(image_embedding_total)

        joint_embedding = self.multi_modal_combine(image_embedding_total, question_embedding_total, image_text_feat_embedding_total)
        logit_res = self.classifier(joint_embedding)
