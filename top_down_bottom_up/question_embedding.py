...
class QuestionEmbedding(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.gru = nn.GRU(input_size=kwargs['embedding_dim'], hidden_size=kwargs['LSTM_hidden_size'],
                          num_layers=kwargs['lstm_layer'], dropout=kwargs['lstm_dropout'], batch_first=True)
...
class AttQuestionEmbedding(nn.Module):
    def __init__(self, num_vocab, **kwargs):
        ...
        if 'embedding_init_file' in kwargs and kwargs['embedding_init_file'] is not None:
            if os.path.isabs(kwargs['embedding_init_file']):
                embedding_file = kwargs['embedding_init_file']
            else:
                embedding_file = os.path.join(cfg.data.data_root_dir, kwargs['embedding_init_file'])
    ...
    def forward(self, input_text):
        ...
        qtt_feature = torch.bmm(qtt_softmax, lstm_drop) # N * conv2_out * LSTM_hidden_size
