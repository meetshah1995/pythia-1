...
from top_down_bottom_up.post_combine_transform import build_post_combine_transform
...
class doubel_project_attention(nn.Module):
    def __init__(self, image_feat_dim, txt_rnn_embeding_dim, hidden_size,dropout=0.2):
        ...
        self.att1 = project_attention(image_feat_dim, txt_rnn_embeding_dim, hidden_size,dropout)
        self.att2 = project_attention(image_feat_dim, txt_rnn_embeding_dim, hidden_size, dropout)
        self.image_feat_dim = image_feat_dim
        self.txt_embeding_dim = txt_rnn_embeding_dim
...
def build_image_attention_module(image_att_model_par, image_dim, ques_dim):
    modal_combine_module = build_modal_combine_module(image_att_model_par['modal_combine']['method'],
                                                      image_att_model_par['modal_combine']['par'],
                                                      image_feat_dim=image_dim, ques_emb_dim=ques_dim)

    transform_module = build_post_combine_transform(image_att_model_par['transform']['method'],
...
class top_down_attention(nn.Module):
    ...
    def _mask_attentions(attention, image_locs):
        ...
        tmp1 = torch.unsqueeze(torch.arange(0, num_loc).type(torch.LongTensor), dim=0).expand(batch_size, num_loc)
